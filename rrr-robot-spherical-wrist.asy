...
void attach_coordinates(transform3 T, real length=2, string suffix="")
{
    ...
    // refined suffix: use braces when not empty
    string sub = (suffix == "") ? "" : "_{" + suffix + "}";

    // labels
    label("$x" + sub + "$", px, T*X, red);
    label("$y" + sub + "$", py, T*Y, green);
    label("$z" + sub + "$", pz, T*Z, blue);
}
...
window.J1 = function(p, t) {
    ...
    let angle = 30 * t1 * Math.PI/180;   // reduced from 90 → 120
...
window.J2 = function(p, t) {
    ...
    let angle = 15 * t2 * Math.PI/180;  // reduced angle
    let cosA = Math.cos(angle), sinA = Math.sin(angle);
...
window.J3 = function(p, t) {
    ...
    let angle = 30 * t3 * Math.PI/180;  // reduced angle
    let cosA = Math.cos(angle), sinA = Math.sin(angle);
...
// Planes, it is the X Y plane
pen bg=gray(0.9);
real r = 2.5;
draw(surface((r,r,0)--(-r,r,0)--(-r,-r,0)--(r,-r,0)--cycle),bg,bg,light=nolight);
// link base
// the L0 should move upward along the Z axis about 2 unit, so
...
// we try to draw the O1 frame, which is static
...
// we must apply the T_0_1 * L0 to all the vertices in the below beginTransform/endTransform block

beginTransform("function(x,t){ return J1(x,t); }", 10);
    // Link 1 start here
    // NOTE: we will start from the identity transform3 I !!!
    transform3 L1_a = joint_r1_m(I, 0);
    
    // we try to draw the O2 frame, which is static(binding to the J1's moving part)
    ...
    beginTransform("function(x,t){ return J2(x,t); }", 10);
        ...
        transform3 L2_a = joint_r2_m(I, 0);  // -30 will make the shoulder up
        
        // we try to draw the O3 frame, which is static(binding to the J1's moving part)
       ...
       beginTransform("function(x,t){ return J3(x,t); }", 10);
            ...
            transform3 L3_a = joint_r1_m(I, 0);
            triple p5 = L3_a*O;
            transform3 L3_b = L3_a*shift(0,0,L3_Len);
            triple p6 = L3_b*O;
            draw(p5--p6, linewidth(2));
            draw_gripper_simple(L3_b);
            transform3 L3_c = L3_b*rotate(-90,Z);
            attach_coordinates(L3_c, 0.5, "4");
        endTransform();
